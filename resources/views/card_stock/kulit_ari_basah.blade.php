...
@section('content')
    ...
    <div class="mainbar">
        <div class="container">
            <div class="header">
                <h2>Card Stock Kulit Ari Basah</h2>
            </div>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                        @foreach ($stokaribasahs as $stokaribasah)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ \Carbon\Carbon::parse($stokaribasah->tanggal)->translatedFormat('d F Y') }}</td>
                                <td>{{ $stokaribasah->remark }}</td>
                                <td>{{ $stokaribasah->begin }}</td>
                                <td>{{ $stokaribasah->in }}</td>
                                <td>{{ $stokaribasah->out }}</td>
                                <td>{{ $stokaribasah->remain }}</td>
                                <td>
                                    <button class="edit" data-id="{{ $stokaribasah->id }}">Edit</button>
                                    <form action="{{ route('card_stock.kulit_ari_basah.destroy', $stokaribasah->id) }}" method="POST"
                                        style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="delete"
                                            data-id="{{ $stokaribasah->id }}">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    document.querySelectorAll('.edit').forEach(button => {
                        button.addEventListener('click', function() {
                            ...
                            fetch(`/card_stock/kulit_ari_basah/${id}/edit`)
