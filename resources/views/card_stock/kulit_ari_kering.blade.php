...
@section('content')
    ...
    <div class="mainbar">
        <div class="container">
            <div class="header">
                <h2>Card Stock Kulit Ari Kering</h2>
            </div>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody>
                        @foreach ($stokarikerings as $stokarikering)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ \Carbon\Carbon::parse($stokarikering->tanggal)->translatedFormat('d F Y') }}</td>
                                <td>{{ $stokarikering->remark }}</td>
                                <td>{{ $stokarikering->begin }}</td>
                                <td>{{ $stokarikering->in }}</td>
                                <td>{{ $stokarikering->out }}</td>
                                <td>{{ $stokarikering->remain }}</td>
                                <td>
                                    <button class="edit" data-id="{{ $stokarikering->id }}">Edit</button>
                                    <form action="{{ route('card_stock.kulit_ari_kering.destroy', $stokarikering->id) }}" method="POST"
                                        style="display: inline;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="delete"
                                            data-id="{{ $stokarikering->id }}">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
            ...
            <div id="modal" class="modal">
                <div class="modal-content">
                    ...
                    <form id="stokForm" action="{{ route('card_stock.kulit_ari_kering.store') }}" method="POST" enctype="multipart/form-data" id="stokForm">
                        @csrf
                        ...
                        <div class="form-group">
                            ...
                            id="activity_type" name="activity_type" value="{{ old('activity_type') }}" required>
                                ...
                                <option value="produksi">Hasil Produksi</option> <!-- stok tambah-->
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    document.querySelectorAll('.edit').forEach(button => {
                        button.addEventListener('click', function() {
                            ...
                            fetch(`/card_stock/kulit_ari_kering/${id}/edit`)
                                ...
                                .then(data => {
                                    ...
                                    form.action = `/card_stock/kulit_ari_kering/${id}`;
